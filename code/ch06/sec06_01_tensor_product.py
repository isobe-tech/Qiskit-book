...
def bitstring_probabilities(state: Statevector) -> dict[str, float]:
    n = int(state.num_qubits)
...
def plot_estimated_distribution(
...
def build_circuits() -> dict[str, QuantumCircuit]:
    ...
    circuit_hh = QuantumCircuit(2, 2)
    circuit_hh.h(0)
    circuit_hh.h(1)
    circuit_hh.measure([0, 1], [0, 1])
    circuits["HH"] = circuit_hh

    circuit_h1 = QuantumCircuit(2, 2)
    circuit_h1.h(1)
    circuit_h1.measure([0, 1], [0, 1])
    circuits["H1"] = circuit_h1
...
def main() -> int:
    ...
    figure_paths = {
        "HH_circuit": "ch06_circuit_hh_measure.pdf",
        "H1_circuit": "ch06_circuit_h1_measure.pdf",
        "HH_hist": "ch06_hist_hh.pdf",
        "H1_hist": "ch06_hist_h1.pdf",
    }

    draw_circuit(circuits["HH"], args.outdir / figure_paths["HH_circuit"])
    draw_circuit(circuits["H1"], args.outdir / figure_paths["H1_circuit"])
    ...
    for key in ["HH", "H1"]:
        ...
        probability = bitstring_probabilities(state)
    ...
    plot_estimated_distribution(
        results["HH"]["estimated_probability"],  # type: ignore[arg-type]
        out_pdf=args.outdir / figure_paths["HH_hist"],
    )
    plot_estimated_distribution(
        results["H1"]["estimated_probability"],  # type: ignore[arg-type]
        out_pdf=args.outdir / figure_paths["H1_hist"],
    )
    ...
    json_path = args.datadir / "ch06_tensor_product_counts.json"
    ...
    for key in ["HH", "H1"]:
        ...
        est = results[key]["estimated_probability"]
        print(f"{key:>3s} counts={counts} estimated={est}")
